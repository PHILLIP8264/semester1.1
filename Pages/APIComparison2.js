...
import SearchField from './SearchFieldComparison2'; 
...
const CardInfo2 = () => {
  ...
  const searchCard = async (searchQuery) => {
    try {
      ...
      setRarities(rarityCounts); // Set the rarities state

      const typeCounts = {}; // Initialize type counts object
      setCards.forEach((card) => {
        const { types } = card;
         // Count types
        if (typeCounts[types]) {
  ...
  const updateChart = () => {
    ...
    const newChartInstance = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label:null,
          data: [parseManaCost(manaCost), parseInt(power), parseInt(toughness)], // Convert mana cost to integer
          backgroundColor: [
            'rgba(255, 99, 132, 0.2)',
            'rgba(54, 162, 235, 0.2)',
            'rgba(255, 206, 86, 0.2)'
...
const createDonutChart = () => {
  ...
  return (
    <div>
      <div className='cards2one'>
        
        <div className='searchcom2'>
          <h1>Card 1</h1>
        ...
        {cardData && (
          <div>
            ...
            <div className='carddata2'>
              <h1>{cardData.name}</h1>
              <div className='space2'>Artist: {authorName}</div> 
              <div className='space2'>Mana Cost: {manaCost}</div>
              <div className='space2'>Power: {power}</div>
              <div className='space2'>Toughness: {toughness}</div>
            </div> 
          </div>
        )}
      </div>
    
      <div className='combargraphy2'>
      <h1 className='comcharthead2'>Mana, Power and Toughness values</h1>
        <canvas ref={chartRef} id="barChart" width="300px" height="300px"></canvas>
      </div>
      
      <div className='compie2'>
        <h1 className='comcharthead2'>Rarity in card's set</h1>
        <canvas ref={pieRef} id="pieChart" ></canvas>
      </div>

      <div className='comdonut2'>
          <h1 className='comcharthead2'>Types in card's set</h1>
